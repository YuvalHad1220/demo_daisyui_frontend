...
export const usePsnrComparison = (key: string): UsePsnrComparisonReturn => {
  ...
  // FIXED: All videos must be ready to be considered loaded
  const allVideosLoaded = useMemo(() => {
    // Only check the remaining codecs (h264, h265, av1)
    const remainingCodecs = ['h264', 'h265', 'av1'];
    const loaded = remainingCodecs.every(codec => readyStates[codec]);
  ...
  // REMOVED: Aggressive buffering monitoring that was causing reset issues
  // The videos will handle their own buffering naturally
  ...
  }, [isPlaying, allVideosLoaded, h264Ref, h265Ref, av1Ref]);
  ...
  }, [currentTime, duration, allVideosLoaded, h264Ref, h265Ref, av1Ref]);
  ...
  }, [allVideosLoaded, h264Ref, h265Ref, av1Ref]);
  ...
  const handleVideoTimeUpdate = useCallback(() => {
    ...
    // }
    // Use the first available video as time source
    if (h264Ref.current) {
      setCurrentTime(h264Ref.current.currentTime);
    }
  }, [h264Ref]);
  ...
  const handleVideoLoadedMetadata = useCallback(() => {
    ...
    // }
    // Use the first available video as duration source
    if (h264Ref.current) {
      setDuration(h264Ref.current.duration);
    }
  }, [h264Ref]);
